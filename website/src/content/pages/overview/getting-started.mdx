...
description: A Step-by-step Guide to Using Sky UI
---

## Quickstart

Running tight on schedule? No worries! Check out our quickstart examples to get started with Sky UI in seconds.
...
The next package you'll need is `@sky-ui/panda-preset`. It extends the Panda Base Preset with advanced color options and opinionated design tokens.

```bash
npm install @sky-ui/panda-preset -D
```
...
Use the Sky UI CLI to add your first component. For example, to add a button, run:

```bash
npx @sky-ui/cli components add button
```
...
```bash
npx @sky-ui/cli components add avatar button card
```

Or use the `--all` flag to install all available components:

```bash
npx @sky-ui/cli components add --all
```
